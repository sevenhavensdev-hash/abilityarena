...
class Database:
    ...
    async def initialize(self):
        ...
        # Older databases used *_roblox for these display names. Rename those
        # columns in place so existing matches remain available after upgrading.
        async with self._db.execute("PRAGMA table_info(matches)") as cur:
            match_columns = {row[1] for row in await cur.fetchall()}
        for old_name, new_name in (
            ("challenger_roblox", "challenger_name"),
            ("opponent_roblox", "opponent_name"),
            ("winner_roblox", "winner_name"),
            ("loser_roblox", "loser_name"),
        ):
            if old_name in match_columns and new_name not in match_columns:
                await self._db.execute(
                    f"ALTER TABLE matches RENAME COLUMN {old_name} TO {new_name}"
                )
                match_columns.remove(old_name)
                match_columns.add(new_name)
                log.info("Renamed matches.%s to matches.%s", old_name, new_name)
    ...
    async def create_match(
        ...
        async with self._lock:
            await self._db.execute(
                """
                INSERT INTO matches
                    (match_id, challenger_id, opponent_id, challenger_name,
                     opponent_name, region, status, match_mode, created_at)
                VALUES (?, ?, ?, ?, ?, ?, 'awaiting', ?, ?)
                """,
                (
                    match_id,
    ...
    async def report_result(
        ...
        async with self._lock:
            await self._db.execute(
                """
                UPDATE matches
                SET reporter_id = ?, winner_id = ?, winner_name = ?,
                    loser_id = ?, loser_name = ?, status = 'awaiting_confirm'
                WHERE match_id = ?
                """,
                (
                    reporter_id,
