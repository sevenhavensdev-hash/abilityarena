...
def get_challenge_channel_id() -> int | None:
    """Read the configured channel used for challenge creation and cleanup."""
    raw_channel_id = os.getenv("CHALLENGE_CHANNEL_ID")
    if not raw_channel_id:
        return None
    try:
        return int(raw_channel_id)
    except ValueError:
        log.warning("CHALLENGE_CHANNEL_ID must be a Discord channel ID.")
        return None
...
class ChallengeModal(discord.ui.Modal, title="⚔️ Create a Duel Challenge"):
    ...
    challenger_name = discord.ui.TextInput(
        label="Your Name",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if interaction.channel_id != get_challenge_channel_id():
            return await interaction.response.send_message(
                "❌ Challenges can only be created in the configured challenge channel.",
                ephemeral=True,
            )

        challenger_name = self.challenger_name.value.strip()
        opponent_name   = self.opponent_name.value.strip()
